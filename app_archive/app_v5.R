...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    card(
      card_header("Evolutions", style = "text-align: center;"),
...
server <- function(input, output, session) {
  ...
  # sprite selected
  output$sprite_selected <- renderImage({
    
    filename <- normalizePath(
      file.path(glue::glue("./sprites/pokemon/front/{evolution_line_ids()[1]}.png")))
    
    # Return a list containing the filename and alt text
    list(src = filename,
         alt = paste("Pokemon Sprite Front"))
